...
class Place:
    ...
    def remove_insect(self, insect):
        ...
        if not insect.is_ant():
            ...
            "*** YOUR CODE HERE ***"
            if insect.name == "Queen" and insect.impostor_queen == False:
                return
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        for _ in range(self.min_range):
            curr_plc = curr_plc.entrance
            
        for _ in range(self.min_range, self.max_range + 1):
            if not curr_plc:
                break
            if random_or_none(curr_plc.bees) and type(curr_plc) != Hive:
                return random_or_none(curr_plc.bees)
            curr_plc = curr_plc.entrance
        return None
